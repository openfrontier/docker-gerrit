...
if [ "$1" = "/gerrit-start.sh" ]; then
  ...
  gosu ${GERRIT_USER} cp -f ${GERRIT_HOME}/importer.jar ${GERRIT_SITE}/plugins/importer.jar
  ...
  if [ "${AUTH_TYPE}" = 'OAUTH' ]  ; then
    gosu ${GERRIT_USER} cp -f ${GERRIT_HOME}/oauth.jar ${GERRIT_SITE}/plugins/oauth.jar
  ...
  #Section gitweb
  case "$GITWEB_TYPE" in
     "gitiles") gosu $GERRIT_USER cp -f $GERRIT_HOME/gitiles.jar $GERRIT_SITE/plugins/gitiles.jar ;;
     "") # Gitweb by default
  ...
  if [ $? -eq 0 ]; then
    ...
    if [ ${NEED_REINDEX} -eq 1 ]; then
      if [ -n "${MIGRATE_TO_NOTEDB_OFFLINE}" ]; then
        ...
        gosu ${GERRIT_USER} java ${JAVA_OPTIONS} ${JAVA_MEM_OPTIONS} -jar "${GERRIT_WAR}" migrate-to-note-db -d "${GERRIT_SITE}"
      else
        echo "Reindexing..."
      ...
      if [ $? -eq 0 ]; then
        ...
        gosu ${GERRIT_USER} echo "${GERRIT_VERSION}" > "${GERRIT_VERSIONFILE}"
